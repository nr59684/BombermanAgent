...
AGENT_ACTIONS_IDX = {'LEFT':0, 'RIGHT':1, 'UP':2, 'DOWN':3, 'WAIT':4, 'BOMB':5}
...
def add_experience(self, old_game_state, self_action, new_game_state, events, n=5):
    ...
    if state_to_features is not None:
        ...
        reward = calculate_reward_from_events(self, events)
        reward += calculate_rewards_from_own_events(self, events)

        # encode the actions into hot-one, needed in train_network()
        action_idx = AGENT_ACTIONS_IDX[self_action]
        ...
        self.experience_buffer.append((features, action, reward, new_features, 0))
...
def train_network(self):
    ...
    for b in sub_batch:
        ...
        y = reward
        if new_features is not None and not isinstance(new_features, type(lambda: None)):
            try:
                print("Newfeatures:",new_features,"type:",type(new_features))
